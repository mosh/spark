...
namespace Spark.Compiler.Oxygene.ChunkVisitors
{
    public class GeneratedCodeVisitor : GeneratedCodeVisitorBase
    {
        ...
        public new void Accept(IList<Chunk> chunks)
        {
            ...
            {
                var chunk = chunks[x];
                if ((chunk is ConditionalChunk) && (x + 1 < chunks.Count) && (chunks[x + 1] is ConditionalChunk) &&
                    (((chunks[x + 1] as ConditionalChunk).Type == ConditionalType.Else) || ((chunks[x + 1] as ConditionalChunk).Type == ConditionalType.ElseIf)))
                {
                    Visit(chunk as ConditionalChunk, false);
                }
                else
                {
                    Accept(chunk);
        ...
        {
            PushScope();
            _source.WriteLine("begin").AddIndent();
        }

        private void AppendCloseBraceNoSemiColon()
        {
            _source.RemoveIndent().WriteLine("end");
            PopScope();
        }
        ...
        protected override void Visit(SendExpressionChunk chunk)
        {
            ...
            {
                _source.WriteLine("except on System.NullReferenceException do");
                AppendOpenBrace();
            ...
            {
                _source.WriteLine("except on ex : System.NullReferenceException do");
                AppendOpenBrace();
        ...
        protected override void Visit(LocalVariableChunk chunk)
        {
            ...
            CodeIndent(chunk).Write("var ").WriteCode(chunk.Name).Write(" : ").WriteCode(chunk.Type).Write(" ");
        ...
        protected override void Visit(ForEachChunk chunk)
        {
            ...
            else
            {
                ...
                if (autoIndex.Detected)
                {
                    ...
                    _source.WriteLine("var {0}Index : Integer := 0;", variableName);
                }
                if (autoIsFirst.Detected)
                {
                    DeclareVariable(variableName + "IsFirst");
                    _source.WriteLine("var {0}IsFirst:Boolean := true;", variableName);
                }
                if (autoCount.Detected)
                {
                    ...
                    _source.WriteLine("var {0}Count : Integer := Spark.Compiler.CollectionUtility.Count({1});", variableName, collectionCode);
                }
                ...
                if (autoIsLast.Detected)
                {
                    ...
                    _source.WriteLine("var {0}IsLast:Boolean := ({0}Index = {0}Count - 1);", variableName);
                }
                //CodeDefault();
                ...
                //CodeHidden();
                if (autoIndex.Detected)
                    _source.WriteLine("inc ({0}Index);", variableName);
                if (autoIsFirst.Detected)
        ...
        {
            CodeIndent(chunk).WriteLine("using(OutputScope('{0}')) do ", chunk.Name); 
            //CodeDefault();
            AppendOpenBrace();
            Accept(chunk.Body);
            AppendCloseBrace();
        ...
        {
            CodeIndent(chunk).WriteLine("using(OutputScope(new System.IO.StringWriter())) do");
            //CodeDefault();
        ...
        {
            CodeIndent(chunk).WriteLine(string.Format("if (Content.ContainsKey('{0}')) then ", chunk.Name));
            //CodeHidden();
            AppendOpenBrace();
        ...
        protected override void Visit(CacheChunk chunk)
        {
            ...
            CodeIndent(chunk)
                ...
                .Write("\", new Spark.CacheExpires(")
                .WriteCode(chunk.Expires)
                .Write("), ")
                .WriteCode(chunk.Key)
                .WriteLine(")) then")
                .WriteLine("begin ").AddIndent();
